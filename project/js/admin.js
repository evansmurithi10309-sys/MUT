...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.toggleAvailability = function(houseId) {
    ...
    if (updated) {
      const statusText = updated.available ? 'Available' : 'Occupied';
      showToast(`House status changed to ${statusText}`, 'success');
      renderAdminDashboard();
    }
  };

  // Open Modal for Adding New House
  if (addHouseBtn) {
    addHouseBtn.addEventListener('click', () => {
      houseFormTitle.textContent = '➕ Add New Vacant House';
      houseForm.reset();
      formHouseId.value = '';
  ...
  window.openEditHouseModal = function(houseId) {
    ...
    formAvailable.checked = house.available !== false;

    uploadedImages = house.images ? [...house.images] : [];
    renderImagePreviews();

    openModal('house-form-modal');
  };

  // Close House Form Modal
  if (houseFormClose) {
  ...
  // Image Upload Handling (Files to Base64)
  if (formImagesUpload) {
  ...
  if (houseForm) {
    houseForm.addEventListener('submit', (e) => {
      ...
      const existingId = formHouseId.value;

      if (existingId) {
        // Edit mode
        DataStore.updateHouse(existingId, houseData);
        showToast('House listing updated successfully!', 'success');
      } else {
        // Add mode
        DataStore.addHouse(houseData);
        showToast('New vacant house added successfully!', 'success');
      }
  ...
  if (deleteConfirmBtn) {
    deleteConfirmBtn.addEventListener('click', () => {
      const houseId = deleteHouseIdInput ? deleteHouseIdInput.value : '';
      if (houseId) {
